...
class HangmanGame(commands.Cog):
    ...
    async def __draw_hangman(self):
        """draw the hangman"""
        with open(WORKING_PATH+"/hangman/hangman_"+str(self.__nb_fails)+".txt", "r") as f:
    ...
    async def start_game(self):
        """Start a game"""
        # Loading replies
        with open(WORKING_PATH+"/hangman/translations/"+self.__language+".json", "r") as f:
            replies = json.load(f)
        ...
        # Choosing a word
        with open(WORKING_PATH+"/hangman/translations/"+self.__language, "r") as f:
            words = f.readlines()
